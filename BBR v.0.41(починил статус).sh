...
enable_bbr() {
    ...
    # Удаляем старые параметры в /etc/sysctl.d
    grep -rl "net.core.default_qdisc" /etc/sysctl.d | while read -r file; do
        safe_sed_remove "net.core.default_qdisc" "$file"
        log_action "Removed conflicting 'net.core.default_qdisc' from $file"
    done

    grep -rl "net.ipv4.tcp_congestion_control" /etc/sysctl.d | while read -r file; do
        safe_sed_remove "net.ipv4.tcp_congestion_control" "$file"
        log_action "Removed conflicting 'net.ipv4.tcp_congestion_control' from $file"
    done

    # Создаём единый файл для настроек
    local bbr_config_file="/etc/sysctl.d/99-bbr.conf"
    echo "net.core.default_qdisc=fq" > "$bbr_config_file"
    echo "net.ipv4.tcp_congestion_control=bbr" >> "$bbr_config_file"
    log_action "BBR settings written to $bbr_config_file"

    # Применяем изменения
    if ! sysctl -p "$bbr_config_file" &>> "$LOG_FILE"; then
...
disable_bbr() {
    ...
    # Удаляем конфликты в /etc/sysctl.d
    grep -rl "net.ipv4.tcp_congestion_control" /etc/sysctl.d | while read -r file; do
        safe_sed_remove "net.ipv4.tcp_congestion_control" "$file"
        log_action "Removed conflicting setting from $file"
    done
