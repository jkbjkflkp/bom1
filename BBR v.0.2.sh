...
function load_type_menu {
    while true; do
        ...
        case $type_choice in
            1)
                echo "Применение настроек для коротких соединений..."
                # Здесь может быть добавлен соответствующий код
                ;;
            2)
                echo "Применение настроек для длительных соединений..."
                apply_long_connections_settings
                ;;
            3)
                echo "Применение стандартных настроек BBR..."
                # Здесь может быть добавлен соответствующий код
                ;;
            4)
                return
...
function apply_long_connections_settings {
    log_file="/var/log/tcp_udp_optimization.log"
